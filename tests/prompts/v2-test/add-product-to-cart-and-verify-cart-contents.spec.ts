...
test.describe.skip("Add Product to Cart and Verify Cart Contents", () => {
  test("Add Product to Cart and Verify Cart Contents", async ({ page }) => {
    ...
    await page
      .locator('[data-test="product-01KHHN305R20NHVT1RBXXK0XVW"]')
      .click();
    ...
    await expect(page.getByText("Combination Pliers")).toBeVisible();
    await expect(page.getByText("$14.15")).toBeVisible();
    await expect(page.locator('[data-test="product-quantity"]')).toHaveValue(
      "2",
    );
